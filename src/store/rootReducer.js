...
import { getData, updateReviewField, updateReview } from '../data/itemdata';
...
const dataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // when you  get the data from firebase
      .addCase(fetchDataFromFirebase.fulfilled, (state, action) => {
        state.status = 'succeeded';
        // data state is equal to all data from firebase 
        state.data = action.payload;
        // filter data by Coffee and Bean
        state.coffeData = state.data.filter((item) => item.type === 'Coffee');
