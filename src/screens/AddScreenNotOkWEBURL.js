...
const AddScreen = ({ navigation }) => {
    ...
    handleSubmitAdd = async (values) => {
        ...
        alert('Im at handleSubmitAdd- line 53 - AddScreen ..!! ');
        ...
        //const storageRef = storage().ref('coffee/imageSquare' + Date.now() + '.jpg'); // For @react-native-firebase/storage

         alert('Im at handleSubmitAdd- line 60 - storageRef: '+storageRef);

        const p = await fetch(imagePortrait)
        const file2 = await p.blob()
        const storageRef2 = ref(storage, 'coffee/imagePortrait' + Date.now() + '.jpg');

         //const storageRef2 = storage().ref('coffee/imagePortrait' + Date.now() + '.jpg'); // For @react-native-firebase/storage

         alert('Im at handleSubmitAdd- line 66 - storageRef: '+storageRef2);
        ...
        /*getDownloadURL(storageRef, values).then((url) => {
            alert('Im at handleSubmitAdd - getDownloadURL- line 79 - url: '+url);
            values.imageLink_square = url;
            alert('Im at handleSubmitAdd- line 80 - values.imageLink_square: '+values.imageLink_square);
        }).catch((error) => {
        console.error('Error getting download URL:', error);
        alert('Error getting download URL - lline 86:', error);
      });
        getDownloadURL(storageRef2, values).then((url) => {
            alert('Im at handleSubmitAdd - getDownloadURL- line 84 - url: '+url);
            values.imageLink_portrait = url;
             alert('Im at handleSubmitAdd- line 84 - values.imageLink_square: '+values.imageLink_portrait);
        }).catch((error) => {
        console.error('Error getting download URL:', error);
        alert('Error getting download URL - lline 93:', error);
      });*/


        values.imageLink_square =  getDownloadURL(storageRef);
        values.imageLink_portrait =   getDownloadURL(storageRef2);

        alert('Product Added, to firebase - line 78 - handleSubmitAdd -values.imageLink_square:  '+values.imageLink_square);
        
        // add values to firebase
        const docRef = await addDoc(collection(db, "Product"), values);
        alert('Product Added, to firebase - line 85 - AddScreen ..!! ');
        setIsLoading(false);
        return;
    }
    return (
        <View style={styles.container}>
            ...
            <Formik
                ...
                onSubmit={(values) => {
          alert("values - line 112 - AddScreen - OnSubmit - values.name : "+ values.name);
          if(!values.name ){
        ...
        if(!values.type ){
           ...
           return;}
        /*if(!values.imageSquare ){
           alert("Need to Enter Product image Square  .... !!!! - Line 140");
           return;} 
        if(!values.imagePortrait ){
           alert("Need to Enter Product image Portrait  .... !!!! - line 143");
           return;}   */


        alert("values - line 142 - AddScreen - Going To handleSubmitAdd ");
